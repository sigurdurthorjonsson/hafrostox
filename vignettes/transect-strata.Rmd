...
Hafró has often emphasized the importance of repeat coverages, `Rstox::surveyPlanner` has an option to add a return trip to the plan from the very beginning, shown as a dashed line on the plot.
```{r,fig.width=8}
...
The objects returned by `surveyPlanner` are list with components: ``r paste(names(prl1),collapse=", ")``. The `Survey` gives a summary of the total including a breakdown of effort allocated on transects and transport legs betweern transects or what sometimes has been referred to as 'transverse transects` or `intersections` at Hafró. For the second parallel design:
...
The `Rstox` package has utilitie for outputting transects in various formats, one of those is `Rstox::writeTransectGPX`. The output is a gpx-file of waypoints, but it would be more natural to work further with gpx-files of routes. 

__todo__
For that purpose we intend to wrap around `hafrostox::gpx_rte`, to be completed.
